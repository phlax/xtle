...
def find_altsrcs(unit, alt_src_langs, store=None, project=None):
    from xtle_store.models import Unit
...
def parse_xtle_revision(store):
    if hasattr(store, "parseheader"):
        xtle_revision = store.parseheader().get(
            "X-Xtle-Revision",
            None)
        if xtle_revision is not None:
            return int(xtle_revision)
    return None
