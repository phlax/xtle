...
class UnitManager(models.Manager):
    ...
    def get_for_user(self, user):
        ...
        # move this to P.accessible_by ?
        user_projects = ProjectSearch(
            user=user, fields=["code"], flat=True).data
...
class StoreManager(models.Manager):
    ...
    def create(self, *args, **kwargs):
        ...
        kwargs["xtle_path"] = (
            "%s%s"
            % (kwargs["parent"].xtle_path, kwargs["name"]))
        kwargs["tp_path"] = (
            "%s%s"
    ...
    def get_or_create(self, *args, **kwargs):
        store, created = super(
            StoreManager, self).get_or_create(*args, **kwargs)
        if not created:
    ...
    def create_by_path(self, xtle_path, project=None,
        ...
        (lang_code, proj_code,
         dir_path, filename) = split_xtle_path(xtle_path)
        ...
        if project is None:
            ...
            raise ValueError(
                "Project must match xtle_path when provided")
