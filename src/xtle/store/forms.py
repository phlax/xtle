...
from xtle.core.delegate import review
from xtle.core.url_helpers import split_xtle_path
from xtle.i18n.gettext import ugettext as _
from xtle.app.models import Directory
from xtle.app.models.permissions import (
    check_permission, check_user_permission)
from xtle.checks.constants import CATEGORY_CODES, CHECK_NAMES
from xtle.comment.forms import UnsecuredCommentForm
from xtle.misc.util import get_date_interval
...
class UnitSearchForm(forms.Form):
    ...
    def clean(self):
        ...
        if self.errors:
            return
        if hasattr(self.request_user, 'get_unit_rows'):
            self.cleaned_data['count'] = self.request_user.get_unit_rows()
        else:
            self.cleaned_data['count'] = 10
        self.cleaned_data["vfolder"] = None
        xtle_path = self.cleaned_data.get("path")
        path_keys = [
            "project_code", "language_code", "dir_path", "filename"]
        try:
            path_kwargs = {
                k: v
                for k, v in resolve(xtle_path).kwargs.items()
                if k in path_keys}
        except Resolver404:
        ...
        self.cleaned_data["sort_by"] = ALLOWED_SORTS[
            sort_on].get(sort_by_param)
    ...
    def clean_path(self):
        lang_code, proj_code = split_xtle_path(
...
class BaseSuggestionForm(UnsecuredCommentForm):
    ...
    def __init__(self, *args, **kwargs):
        kwargs["request_user"] = kwargs.get(
            "request_user") or self.request_user
...
class SubmitFormMixin(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields[
            "target_f"].hidden_widget = HiddenMultiStringWidget(
                nplurals=nplurals)
        self.fields["target_f"].fields = [
            forms.CharField(strip=False, required=False)
