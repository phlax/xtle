...
from xtle.core.delegate import config, serializers
...
class StoreSerialization(object):
    ...
    def tostring(self, include_obsolete=False, raw=False):
        ...
        if hasattr(store, "updateheader"):
            ...
            store.updateheader(add=True, X_Xtle_Path=self.xtle_path)
            store.updateheader(add=True, X_Xtle_Revision=max_unit_revision)
