...
from xtle.core.log import STORE_DELETED, STORE_OBSOLETE, store_log
from xtle.core.models import Revision
from xtle.core.search import SearchBroker
from xtle.core.signals import toggle, update_checks, update_data
from xtle.core.url_helpers import (
    get_editor_filter, split_xtle_path, to_tp_relative_path)
from xtle.core.utils import dateformat
from xtle.core.utils.aggregate import max_column
from xtle.core.utils.multistring import PLURAL_PLACEHOLDER
from xtle.core.utils.timezone import datetime_min
from xtle.checks.constants import CHECK_NAMES
from xtle.statistics.models import SubmissionFields, SubmissionTypes
...
from .constants import (
    DEFAULT_PRIORITY, FUZZY, OBSOLETE, XTLE_WINS, TRANSLATED, UNTRANSLATED)
...
class Suggestion(AbstractSuggestion):
    """Suggested translation for a :cls:`~xtle_store.models.Unit`, provided
...
class Unit(AbstractUnit):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if "source_f" in self.__dict__ and "target_f" in self.__dict__:
    ...
    def is_accessible_by(self, user):
        ...
        from xtle_project.models import Project
    ...
    def update(self, unit, user=None):
        ...
        developer_comment = (
            ...
            and (self.developer_comment or notes))
        ...
        translator_comment = (
            ...
            and (self.translator_comment or notes))
        ...
        # this is problematic - it compares getid,
        # but then sets getid *or* source
        if self.unitid != unit.getid():
            self.unitid = unit.getid() or str(unit.source)
    ...
    def update_qualitychecks(self, keep_false_positives=False):
        ...
        for name in qc_failures.iterkeys():
            if name in existing:
                ...
                unmute = (
                    existing[name]['false_positive']
                    and not keep_false_positives)
                if unmute:
...
class Store(AbstractStore):
    ...
    def has_terminology(self):
        ...
        return self.name.startswith('xtle-terminology')
    ...
    def path(self):
        ...
        If the `xtle_path` of a :cls:`Store` object `store` is
        `/af/project/dir1/dir2/file.po`, `store.path` will return
        `dir1/dir2/file.po`.
        """
        return to_tp_relative_path(self.xtle_path)
    ...
    def save(self, *args, **kwargs):
        self.xtle_path = self.parent.xtle_path + self.name
    ...
    def delete(self, *args, **kwargs):
        store_log(user='system', action=STORE_DELETED,
                  path=self.xtle_path, store=self.id)
