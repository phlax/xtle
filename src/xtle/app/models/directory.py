...
from xtle.misc.baseurl import link
...
class DirectoryManager(models.Manager):
    ...
    @cached_property
    def root(self):
        return self.get(xtle_path='/')

    @cached_property
    def projects(self):
        return self.get(xtle_path='/projects/')
...
class Directory(models.Model, CachedTreeItem):
    ...
    parent = models.ForeignKey('xtle_app.Directory', related_name='child_dirs',
                               null=True, blank=True, db_index=True,
                               on_delete=models.CASCADE)
    # any changes to the `xtle_path` field may require updating the schema
    # see migration 0005_case_sensitive_schema.py
    xtle_path = models.CharField(
        max_length=255,
        null=False,
        db_index=True,
        unique=True,
    ...
    def get_absolute_url(self):
        return link(self.xtle_path)

    def get_translate_url(self, **kwargs):
        lang_code, proj_code, dir_path = split_xtle_path(self.xtle_path)[:3]

        if lang_code and proj_code:
            pattern_name = 'xtle-tp-translate'
            pattern_args = [lang_code, proj_code, dir_path]
        elif lang_code:
            pattern_name = 'xtle-language-translate'
    ...
    def trail(self, only_dirs=True):
        ...
        :cls:`~xtle_translationproject.models.TranslationProject` and above.
        """
        path_parts = self.xtle_path.split('/')
        parents = []
        if only_dirs:
        ...
        if parents:
            return Directory.objects.live().filter(xtle_path__in=parents) \
                                           .order_by('xtle_path')
    ...
    def is_language(self):
        ...
        return (
            self.xtle_path.count('/') == 2
            and not self.xtle_path.startswith('/projects/'))

    def is_project(self):
        return (
            self.xtle_path.startswith('/projects/')
            and self.xtle_path.count('/') == 3)

    def is_translationproject(self):
        """does this directory point at a translation project"""
        return (self.xtle_path.count('/') == 3 and not
                self.xtle_path.startswith('/projects/'))

    def is_projects_root(self):
        """is this directory a projects root directory"""
        return self.xtle_path == '/projects/'
