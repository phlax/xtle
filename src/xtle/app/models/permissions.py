...
def get_xtle_permission(codename):
    # The content type of our permission
    content_type = get_permission_contenttype()
    # Get the xtle view permission
    return Permission.objects.get(content_type=content_type, codename=codename)


def get_permissions_by_user(user, directory):
    xtle_path = directory.xtle_path
    path_parts = filter(None, xtle_path.split('/'))
    try:
        permissionset = user.permissionset_set.select_related(
            "directory").filter(
                directory__in=directory.trail(
                    only_dirs=False)).order_by(
                        '-directory__xtle_path')[0]
    except IndexError:
        permissionset = None
        ...
        (len(path_parts) > 1
         ...
         and (permissionset is None
              or len(
                  filter(
                      None,
                      permissionset.directory.xtle_path.split('/'))) < 2)))
    ...
    if check_project_permissions:
        ...
        try:
            ...
            permissionset = user.permissionset_set.select_related(
                "directory").get(
                    directory__xtle_path=project_path)
...
class PermissionSet(models.Model):
    ...
    directory = models.ForeignKey('xtle_app.Directory', db_index=True,
