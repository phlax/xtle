...
from xtle.app.runner import set_sync_mode
...
class XtleCommand(BaseCommand):
    """Base class for handling recursive xtle store management commands."""
    ...
    def _handle(self, **options):
        ...
        logger.info('[xtle] Running: %s', self.name)

        self.handle_all(**options)

        # info finish
        end = datetime.datetime.now()
        logging.info('[xtle] Complete: %s in %s', self.name, end - start)
