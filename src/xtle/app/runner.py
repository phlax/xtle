...
from xtle.core.cache import PERSISTENT_STORES
from xtle.core.log import cmd_log
...
SYSTEM_SETTINGS_PATH = os.path.join('/etc', 'xtle', 'xtle.conf')
HOME_SETTINGS_PATH = os.path.join('~', '.xtle', 'xtle.conf')
if "VIRTUAL_ENV" in os.environ:
    VENV_SETTINGS_PATH = os.path.join(os.environ["VIRTUAL_ENV"], "xtle.conf")
else:
    VENV_SETTINGS_PATH = ""
...
def init_settings(settings_filepath, template_filename,
                  db="sqlite", db_name="dbs/xtle.db", db_user="",
...
def init_command(parser, args):
    """Parse and run the `xtle init` command
    ...
    parser.add_argument(
        "--db-name", default="",
        help=(u"Database name (default: 'xtledb') or path "
              u"to database file if using sqlite (default: "
              u"'%s/dbs/xtle.db')" % src_dir))
    parser.add_argument(
        "--db-user", default="",
...
def run_app(project, django_settings_module):
    ...
    # ImproperlyConfigured error on trying to run any xtle commands
    ...
    # Print the help message for "xtle --help"
...
def main():
    run_app(project='xtle', django_settings_module='xtle.app.settings')
