...
from xtle.store.constants import FUZZY, TRANSLATED, UNTRANSLATED
...
class SuggestionCreatedScore(BaseSuggestionScore):
    score_setting = settings.XTLE_SCORES['suggestion_add']
...
class SuggestionAcceptedScore(SuggestionReviewScore):
    score_setting = settings.XTLE_SCORES['suggestion_accept']


class SuggestionRejectedScore(SuggestionReviewScore):
    score_setting = settings.XTLE_SCORES['suggestion_reject']


class TargetUpdatedScore(EventScore):
    score_setting = settings.XTLE_SCORES['target_updated']
...
class StateUpdatedScore(SubmissionEventScore):
    ...
    def score_setting(self):
        if self.translated:
            return settings.XTLE_SCORES['state_translated']
        elif self.state_fuzzied:
            return settings.XTLE_SCORES['state_fuzzy']
        elif self.state_unfuzzied:
            return settings.XTLE_SCORES['state_unfuzzy']


class CommentUpdatedScore(SubmissionEventScore):
    score_setting = settings.XTLE_SCORES['comment_updated']
