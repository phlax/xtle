...
from xtle.core.utils.version import get_major_minor_version
from xtle.statistics.models import Submission
from xtle.store.syncer import StoreSyncer
...
class PoStoreSyncer(StoreSyncer):
    ...
    def get_po_headers(self, mtime, user_displayname, user_email):
        headerupdates = {
            ...
            'X_Generator': "Xtle %s" % get_major_minor_version(),
            'X_XTLE_MTIME': self.get_po_mtime(mtime)}
        headerupdates['Last_Translator'] = (
            user_displayname and user_email
            and ('%s <%s>'
                 % (user_displayname,
                    user_email))
            or 'Anonymous Xtle User')
        return headerupdates

    def update_po_headers(self, disk_store, mtime,
