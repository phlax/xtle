...
from xtle.core.bulk import BulkCRUD
from xtle.core.signals import create, update
...
class RevisionUpdater(object):
    ...
    def object_list_paths(self):
        return set(
            self.object_list.values_list(
                self.related_xtle_path,
                flat=True))

    @property
    def all_xtle_paths(self):
    ...
    def parents(self):
        ...
        return Directory.objects.filter(
            xtle_path__in=self.get_parent_paths(self.all_xtle_paths))
...
class UnitRevisionUpdater(RevisionUpdater):
    related_xtle_path = "store__parent__xtle_path"

    @property
    def context_path(self):
        return self.context.store.parent.xtle_path


class StoreRevisionUpdater(RevisionUpdater):
    related_xtle_path = "parent__xtle_path"

    @property
    def context_path(self):
        return self.context.parent.xtle_path


class DirectoryRevisionUpdater(RevisionUpdater):
    related_xtle_path = "xtle_path"

    @property
    def context_path(self):
        return self.context.xtle_path
...
class ProjectRevisionUpdater(RevisionUpdater):
    ...
    def get_parent_paths(self, xtle_paths):
        ...
        tps = TranslationProject.objects.filter(
            project__code__in=projects).values_list(
