...
from xtle.checks.constants import EXCLUDED_FILTERS
...
class TranslationProjectManager(models.Manager):
    ...
    def get_terminology_project(self, language_id):
        ...
        # (see xtle_project/models.py:240).
        #
...
class TranslationProject(models.Model, CachedTreeItem):
    ...
    language = models.ForeignKey(
        "xtle_language.Language",
        db_index=False,
        on_delete=models.CASCADE)
    project = models.ForeignKey(
        "xtle_project.Project",
        db_index=True,
        on_delete=models.CASCADE)
    ...
    def save(self, *args, **kwargs):
        ...
        self.xtle_path = self.directory.xtle_path
    ...
    def create_parent_dirs(self, xtle_path):
        parent = self.directory
        dirs_to_create = []
        for path in PurePosixPath(xtle_path).parents:
            path = posixpath.join(str(path), "")
            if path == self.xtle_path:
                break
    ...
    def init_store_from_template(self, template_store):
        ...
        """
        xtle_path = posixpath.join(
            self.xtle_path.rstrip("/"),
            template_store.tp_path.lstrip("/"))
        xtle_path = ".".join(
            [posixpath.splitext(xtle_path)[0],
             template_store.filetype.extension.name])
        name = posixpath.basename(xtle_path)
        if self.project.is_gnustyle:
            ...
            name = ".".join(
                [self.language.code,
                 template_store.filetype.extension.name])
            dirname = posixpath.dirname(xtle_path)
            xtle_path = posixpath.join(dirname, name)
        if not self.stores.filter(xtle_path=xtle_path).exists():
            return self.stores.create(
                parent=self.create_parent_dirs(xtle_path),
                xtle_path=xtle_path,
                name=name)
