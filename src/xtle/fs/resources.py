...
class FSProjectResources(object):
    ...
    def excluded_languages(self):
        return self.project.config.get("xtle.fs.excluded_languages")
...
class FSProjectStateResources(object):
    ...
    Accepts `xtle_path` and `fs_path` glob arguments.
    If present all resources are filtered accordingly.
    """

    sw_version = XTLEFSConfig.version
    ns = "xtle.fs.resources"

    def __init__(self, context, xtle_path=None, fs_path=None):
    ...
    def found_file_matches(self):
        return sorted(self.context.find_translations(
            fs_path=self.fs_path, xtle_path=self.xtle_path))
    ...
    def trackable_stores(self):
        ...
        for store in stores:
            fs_path = self.match_fs_path(
                self.context.get_fs_path(store.xtle_path))
            if fs_path:
    ...
    def trackable_store_paths(self):
        """Dictionary of xtle_path, fs_path for trackable Stores"""
        return {
            store.xtle_path: fs_path
            for store, fs_path
    ...
    def _tracked_paths(self):
        ...
        return dict(self.tracked.values_list("path", "xtle_path"))
    ...
    @cached_property
    def xtle_revisions(self):
        return dict(
            self.synced.exclude(store_id__isnull=True)
                       .exclude(store__obsolete=True)
                       .values_list(
                           "store_id",
                           "store__data__max_unit_revision"))
    ...
    def file_hashes(self):
        ...
        for xtle_path, path in self.found_file_matches:
            hashes[xtle_path] = _get_file_hash(path)
        return hashes
    ...
    def fs_changed(self):
        ...
        for store_fs in self.synced.iterator():
            if store_fs.last_sync_hash == hashes.get(store_fs.xtle_path):
                continue
