...
from xtle.core.delegate import response, state
...
@getter(
    [config_should_not_be_set,
     config_should_not_be_appended],
    sender=Project)
def fs_url_config_validator(**kwargs):
    if kwargs["key"] == "xtle_fs.fs_type":
        plugin_class = fs_plugins.gather(Project).get(kwargs["value"])
        if not plugin_class:
            raise ValidationError("Unrecognised fs_type")
    if kwargs["key"] == "xtle_fs.fs_url":
        fs_type = kwargs["instance"].config.get("xtle_fs.fs_type")
        if not fs_type:
    ...
    if kwargs["key"] == "xtle_fs.translation_mapping":
