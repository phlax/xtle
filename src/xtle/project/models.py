...
from xtle.core.cache import make_method_key
from xtle.core.delegate import data_tool, filetype_tool, lang_mapper, tp_tool
from xtle.core.mixins import CachedTreeItem
from xtle.core.models import VirtualResource
from xtle.core.url_helpers import get_editor_filter, split_xtle_path
from xtle.i18n.gettext import ugettext_lazy as _
from xtle.app.models.directory import Directory
from xtle.app.models.permissions import PermissionSet
from xtle.language.models import Language
from xtle.config.utils import ObjectConfig
from xtle.format.models import Format
from xtle.format.utils import ProjectFiletypes
from xtle.revision.models import Revision
from xtle.staticpages.models import StaticPage
...
class ProjectManager(models.Manager):
    ...
    def create(self, *args, **kwargs):
        ...
        project.config["xtle_fs.fs_type"] = kwargs.pop("fs_type", "localfs")
        if project.config["xtle_fs.fs_type"] == "localfs":
            project.config["xtle_fs.fs_url"] = kwargs.pop(
                "fs_url",
                "{XTLE_TRANSLATION_DIRECTORY}%s" % project.code)
        return project
    ...
    def cached_dict(self, user):
        ...
        if not projects:
            ...
            cache.set(cache_key, projects, settings.XTLE_CACHE_TIMEOUT)
...
class Project(models.Model, CachedTreeItem, ProjectURLMixin):
    ...
    filetypes = SortedManyToManyField("xtle_format.Format")

    source_language = models.ForeignKey(
        Language, db_index=True,
    ...
    directory = models.OneToOneField(
        "xtle_app.Directory", db_index=True, editable=False,
    ...
    def accessible_by_user(cls, user):
        ...
        if user.is_superuser:
            ...
            if user.is_anonymous:
                allow_usernames = ["anon"]
                forbid_usernames = ["anon", 'default']
            else:
            ...
            public_projects = cls.objects.filter(
                public=True,
            ).values_list('code', flat=True)

            accessible_projects = cls.objects.filter(**{
                ("directory__permission_sets__"
                 "positive_permissions__codename"): "view",
                ("directory__permission_sets__"
                 "user__username__in"): allow_usernames,
            }).values_list('code', flat=True)
            ...
            forbidden_projects = cls.objects.filter(**{
                ("directory__permission_sets__"
                 "negative_permissions__codename"): 'hide',
                ("directory__permission_sets__"
                 "user__username__in"): forbid_usernames,
            }).values_list('code', flat=True)

            allow_projects = set(accessible_projects) | set(public_projects)
        ...
        cache.set(key, user_projects, settings.XTLE_CACHE_TIMEOUT)
    ...
    def local_fs_path(self):
        return os.path.join(settings.XTLE_FS_WORKING_PATH, self.code)
    ...
    @cached_property
    def languages(self):
        """Returns a list of active :cls:`~xtle_languages.models.Language`
        objects for this :cls:`~xtle_project.models.Project`.
        """
        from xtle_language.models import Language
    ...
    def delete(self, *args, **kwargs):
        ...
        # xtle_store suggestions, xtle_app suggestions and submissions.
    ...
    def is_gnustyle(self):
        mapping = self.config[
            "xtle_fs.translation_mappings"]["default"]
        return bool(
            "/" not in mapping[
    ...
    def get_announcement(self, user=None):
        ...
        return StaticPage.get_announcement_for(self.xtle_path, user)
...
class ProjectResource(VirtualResource, ProjectURLMixin):
    ...
    def __eq__(self, other):
        return (
            self.xtle_path == other.xtle_path
...
class ProjectSet(VirtualResource, ProjectURLMixin):
    ...
    def __init__(self, resources, *args, **kwargs):
        self.directory = Directory.objects.projects
        super(ProjectSet, self).__init__(resources, self.directory.xtle_path)
...
def invalidate_accessible_projects_cache(**kwargs):
    ...
    dont_invalidate = (
        instance.__class__.__name__ not in
