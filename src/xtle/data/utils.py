...
from xtle.core.delegate import data_updater, revision
from xtle.core.signals import create, delete, update
from xtle.core.url_helpers import split_xtle_path
from xtle.statistics.models import Submission
from xtle.statistics.proxy import SubmissionProxy
from xtle.store.models import Unit

from .apps import XTLEDataConfig
...
class RelatedStoresDataTool(DataTool):
    ...
    def aggregate_children(self, stats):
        ...
        for path, child in stats["children"].items():
            if path.startswith("templates-") or path == "templates":
                if not self.context.xtle_path.startswith("/templates/"):
            ...
            if child.get("last_created_unit"):
                ...
                last_created_time = last_created.get("creation_time", 0)
                if (last_created_time > (last_created_unit_time or 0)):
                    latest["last_created_unit"] = last_created
                    last_created_unit_time = last_created_time
            if (child["last_submission__pk"] or 0) > (last_submission_pk or 0):
    ...
    def get_stats(self, include_children=True, aggregate=True, user=None):
        ...
        add_untranslated = (
            ...
            and not self.context.xtle_path.startswith("/templates/"))
        if add_untranslated:
            stats["untranslated"] = stats["total"] - stats["translated"]
            stats["incomplete"] = stats["total"] - stats["translated"]
        return stats
    ...
    def add_child_stats(self, children, child, root=None, use_aggregates=True):
        """For a child member of children qs.values, add the childs stats to
        aggregate stats for the child' root node (ie the parent node which is
        the immediate descendant of the context).
        """
