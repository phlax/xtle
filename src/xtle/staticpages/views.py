...
from xtle.core.http import JsonResponse, JsonResponseBadRequest
from xtle.core.markup.filters import apply_markup_filter
from xtle.core.views.mixins import SuperuserRequiredMixin
from xtle.i18n.gettext import ugettext_lazy as _
from xtle_misc.util import ajax_required
...
class PageModelMixin(object):
    ...
    def form_valid(self, form):
        vpath = (
            self.page_type == ANN_TYPE
            and not form.cleaned_data[
                'virtual_path'].startswith(ANN_VPATH))
        if vpath:
...
class PageCreateView(SuperuserRequiredMixin, AdminCtxMixin, PageModelMixin,
    ...
    success_url = reverse_lazy('xtle-staticpages')
