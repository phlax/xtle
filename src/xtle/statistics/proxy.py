...
from xtle.core.primitives import PrefixedDict
...
class SubmissionProxy(object):
    ...
    @property
    def unit_xtle_path(self):
        return self.values.get("unit__store__xtle_path")
    ...
    def unit_info(self):
        ...
        info.update(
            dict(check_name=self.qc_name,
                 check_display_name=CHECK_NAMES.get(
                     self.qc_name,
                     self.qc_name),
                 checks_url=reverse(
                     'xtle-checks-descriptions')))
        return info
