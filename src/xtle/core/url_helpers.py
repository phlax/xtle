...
def split_xtle_path(xtle_path):
    """Split an internal `xtle_path` into proper parts.

    :return: A tuple containing each part of a xtle_path`::
        (language code, project code, directory path, filename)
    """
    slash_count = xtle_path.count(u'/')
    parts = xtle_path.split(u'/', 3)[1:]

    language_code = None
    project_code = None
    ctx = ''

    if slash_count != 0 and xtle_path != '/projects/':
        # /<lang_code>/
        if slash_count == 2:
            language_code = parts[0]
        # /projects/<project_code>/
        elif xtle_path.startswith('/projects/'):
...
def to_tp_relative_path(xtle_path):
    """Returns a path relative to translation projects.

    If `xtle_path` is `/af/project/dir1/dir2/file.po`, this will
    return `dir1/dir2/file.po`.
    """
    return u'/'.join(xtle_path.split(u'/')[3:])
...
def get_previous_url(request):
    ...
    if referer_url:
        parsed_referer = urllib.parse(referer_url)
